function gadget:GetInfo()
	return {
		...
		desc = "Evolves Commanders based on xp",
		author = "[Fx]Doo",
...
if (gadgetHandler:IsSyncedCode()) then
    ...
    function gadget:UnitCreated(unitID) -- add COMMANDER[unitID] and ARMCOMMANDER2[unitID] upon creation
		...
		if unitName == "armcom3" then
            ARMCOMMANDER3[unitID] = true
			COMMANDERS[unitID] = true
        end
		if unitName == "armcom4" then
            ARMCOMMANDER4[unitID] = true
			COMMANDERS[unitID] = true
        end
		if unitName == "armcom5" then
            ARMCOMMANDER5[unitID] = true
			COMMANDERS[unitID] = true
        end
		if unitName == "armcom6" then
		...
		if unitName == "corcom3" then
            CORCOMMANDER3[unitID] = true
			COMMANDERS[unitID] = true
        end
		if unitName == "corcom4" then
            CORCOMMANDER4[unitID] = true
			COMMANDERS[unitID] = true
        end
		if unitName == "corcom5" then
            CORCOMMANDER5[unitID] = true
			COMMANDERS[unitID] = true
        end
		if unitName == "corcom6" then
	...
	-- Spring.Echo("working")
		...
		realxp = 10 * fxp
			...
			newid = Spring.CreateUnit("armcom2", x, y, z, 0, team, false, false)
				if (newid) then
		...
		realxp = 10 * fxp
			...
			rx, ry, rz = Spring.GetUnitRotation(unitID)
			team = Spring.GetUnitTeam(unitID)
			newid = Spring.CreateUnit("armcom3", x, y, z, 0, team, false, false)
				if (newid) then
					Spring.DestroyUnit(unitID, false, true)
					Spring.SetUnitHealth(newid, {health = hp, capture = capture, paralyze = para})
					Spring.SetUnitRotation(newid, rx, ry, rz)
		...
		realxp = 10 * fxp
			...
			rx, ry, rz = Spring.GetUnitRotation(unitID)
			team = Spring.GetUnitTeam(unitID)
			newid = Spring.CreateUnit("armcom4", x, y, z, 0, team, false, false)
				if (newid) then
					Spring.DestroyUnit(unitID, false, true)
					Spring.SetUnitHealth(newid, {health = hp, capture = capture, paralyze = para})
					Spring.SetUnitRotation(newid, rx, ry, rz)
		...
		realxp = 10 * fxp
			...
			rx, ry, rz = Spring.GetUnitRotation(unitID)
			team = Spring.GetUnitTeam(unitID)
			newid = Spring.CreateUnit("armcom5", x, y, z, 0, team, false, false)
				if (newid) then
					Spring.DestroyUnit(unitID, false, true)
					Spring.SetUnitHealth(newid, {health = hp, capture = capture, paralyze = para})
					Spring.SetUnitRotation(newid, rx, ry, rz)
		...
		realxp = 10 * fxp
			...
			rx, ry, rz = Spring.GetUnitRotation(unitID)
			team = Spring.GetUnitTeam(unitID)
			newid = Spring.CreateUnit("armcom6", x, y, z, 0, team, false, false)
				if (newid) then
					Spring.DestroyUnit(unitID, false, true)
					Spring.SetUnitHealth(newid, {health = hp, capture = capture, paralyze = para})
					Spring.SetUnitRotation(newid, rx, ry, rz)
		...
		realxp = 10 * fxp
			...
			rx, ry, rz = Spring.GetUnitRotation(unitID)
			team = Spring.GetUnitTeam(unitID)
			newid = Spring.CreateUnit("corcom2", x, y, z, 0, team, false, false)
				if (newid) then
					Spring.DestroyUnit(unitID, false, true)
					Spring.SetUnitHealth(newid, {health = hp, capture = capture, paralyze = para})
					Spring.SetUnitRotation(newid, rx, ry, rz)
		...
		realxp = 10 * fxp
			...
			rx, ry, rz = Spring.GetUnitRotation(unitID)
			team = Spring.GetUnitTeam(unitID)
			newid = Spring.CreateUnit("corcom3", x, y, z, 0, team, false, false)
				if (newid) then
					Spring.DestroyUnit(unitID, false, true)
					Spring.SetUnitHealth(newid, {health = hp, capture = capture, paralyze = para})
					Spring.SetUnitRotation(newid, rx, ry, rz)
		...
		realxp = 10 * fxp
			...
			rx, ry, rz = Spring.GetUnitRotation(unitID)
			team = Spring.GetUnitTeam(unitID)
			newid = Spring.CreateUnit("corcom4", x, y, z, 0, team, false, false)
				if (newid) then
					Spring.DestroyUnit(unitID, false, true)
					Spring.SetUnitHealth(newid, {health = hp, capture = capture, paralyze = para})
					Spring.SetUnitRotation(newid, rx, ry, rz)
		...
		realxp = 10 * fxp
			...
			rx, ry, rz = Spring.GetUnitRotation(unitID)
			team = Spring.GetUnitTeam(unitID)
			newid = Spring.CreateUnit("corcom5", x, y, z, 0, team, false, false)
				if (newid) then
					Spring.DestroyUnit(unitID, false, true)
					Spring.SetUnitHealth(newid, {health = hp, capture = capture, paralyze = para})
					Spring.SetUnitRotation(newid, rx, ry, rz)
		...
		realxp = 10 * fxp
			...
			rx, ry, rz = Spring.GetUnitRotation(unitID)
			team = Spring.GetUnitTeam(unitID)
			newid = Spring.CreateUnit("corcom6", x, y, z, 0, team, false, false)
				if (newid) then
					Spring.DestroyUnit(unitID, false, true)
					Spring.SetUnitHealth(newid, {health = hp, capture = capture, paralyze = para})
					Spring.SetUnitRotation(newid, rx, ry, rz)
