...
--SYNCED
	...
	function gadget:GameFrame(frame)
		...
		-- Shouldn't be needed anymore with this new version of the gadget
		...
		for _,msg in pairs(RecMsg) do
			...
			if u and Spring.ValidUnitID(u) then
				...
				if not fa and frame>=Spring.GetUnitWeaponState(u,Game.gameName and 1 or 0,"reloadFrame") then-- Since 95 , weapon index changed
					Spring.SetUnitWeaponState(u,Game.gameName and 1 or 0,"reloadFrame",frame+1) -- Assuming one and only one weapon
				end
...
local GetSelectedUnits        = Spring.GetSelectedUnits
	local function Unsynced_GameFrame(caller,frame)
		local u=GetTheOne()
			if Spring.GetKeyState(44) then --111 down o
		...
		if u then
			...
			-- Get keyboard input
				if Spring.GetKeyState(276) or Spring.GetKeyState(97) then --left 
					MoveX=MoveX-1
				end
				...
				if Spring.GetKeyState(273)or Spring.GetKeyState(119)  then --up 
					MoveZ=MoveZ-1
				end
			...
			if u then
				...
				if Spring.GetKeyState(46) then --down o
					Spring.Echo("1")
					msg=msg.." SB"
