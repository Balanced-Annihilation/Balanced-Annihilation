local torso, ruparm, luparm, rbigflash, nanospray, pelvis, lfirept, head, lthigh, rthigh, nanolath, biggun, rleg, lleg, rdirt, ldirt, lfoot, rfoot = piece ("torso", "ruparm", "luparm", "rbigflash", "nanospray", "pelvis", "lfirept", "head", "lthigh", "rthigh", "nanolath", "biggun", "rleg", "lleg", "rdirt", "ldirt", "lfoot", "rfoot")
local piecetable = {torso, ruparm, luparm, rbigflash, nanospray, pelvis, lfirept, head, lthigh, rthigh, nanolath, biggun, rleg, lleg, rdirt, ldirt, lfoot, rfoot}
local SIG_WALK = 2
...
function walk()
		...
		Turn (nanolath, 1, GetAngleSpeed(-5-Addangle/2,15))
		...
		Turn (nanolath, 1, GetAngleSpeed(-5-Addangle/2,5))
		...
		Turn (nanolath, 1, GetAngleSpeed(-5-Addangle/2,10))
		...
		Turn (rfoot, 1, GetAngleSpeed(-Addangle,45))
		Turn (luparm, 1, GetAngleSpeed(-15-Addangle/2,15))
		Turn (nanolath, 1, GetAngleSpeed(-15-Addangle/2,15))
		Turn (ruparm, 1, GetAngleSpeed(15-Addangle/2,15))
		...
		Turn (nanolath, 1, GetAngleSpeed(-30-Addangle/2,15))
		...
		Turn (nanolath, 1, GetAngleSpeed(-5-Addangle,30))
...
function UnitSpeed()
	...
	while (true) do
		...
		Sleep (1)
		local lx,ly,lz = Spring.GetUnitPiecePosDir(unitID, ldirt) 
		-- local rx,ry,rz = Spring.GetUnitPiecePosDir(unitID, rdirt)
		-- local lh = Spring.GetGroundHeight(lx,lz)
		-- local rh = Spring.GetGroundHeight(rx,rz)
					-- Spring.Echo((lh))
			-- Spring.Echo((ly))
		-- if ly < lh or ry < rh then
			-- if math.abs(lh - ly) > math.abs(rh - ry) then

				-- Move (pelvis, 2, math.abs(lh-2-ly),1)
				-- addheight = math.abs(lh-ly)
				-- else
				-- Move (pelvis, 2, math.abs(rh-2-ry),1)
				-- addheight = math.abs(rh-ry)
			-- end
		-- end
...
if dgunning == true then
	...
	Turn (luparm, 1, (-1.57-pitch)/4, 250*0.0174533 * 2)
	Turn (nanolath, 1, (-1.57-pitch)*3/4, 250*0.0174533 * 2)
	Turn (ruparm, 1, (-1.57-pitch)/8, 250*0.0174533 * 2)
	Turn (ruparm, 3, GetAngleSpeed(0,4))
	Turn (biggun, 1, (-1.57-pitch)*3/8, 250*0.0174533 * 2)
	WaitForTurn (torso, 2)
	WaitForTurn (torso, 1)
...
function script.AimWeapon3( heading, pitch )
	...
	Turn (torso, 1, GetAngleSpeed(0,4))
	Turn (torso, 2, heading, 3)
	Turn (luparm, 1, (-1.57-pitch)/8, 250*0.0174533 * 2)
	Turn (nanolath, 1, (-1.57-pitch)*3/8, 250*0.0174533 * 2)
