...
local SSAO_KERNEL_SIZE = 12 -- how many samples are used for SSAO spatial sampling
...
local SSAO_ALPHA_POW = 64.0 -- consider this as SSAO effect strength
...
local BLUR_SIGMA = 3.5 -- Gaussian sigma of a single blur pass, other factors like BLUR_HALF_KERNEL_SIZE, BLUR_PASSES and DOWNSAMPLE affect the end result gaussian shape too

local DOWNSAMPLE = 1 -- increasing downsapling will reduce GPU RAM occupation (a little bit), increase performace (a little bit), introduce occlusion blockiness
