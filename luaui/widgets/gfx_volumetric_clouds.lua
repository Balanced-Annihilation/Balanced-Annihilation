...
local minheight = ((Game.mapSizeX +Game.mapSizeZ)/2)/4
local minheightAdjusted = minheight*2.6
local scalemult = 1.5

if(Game.mapSizeX ==Game.mapSizeZ)then
	minheight = ((Game.mapSizeX +Game.mapSizeZ)/2)/4
	minheightAdjusted = minheight*1.6
	scalemult = 2
end
...
local CloudDefs = {
	...
	height   = minheightAdjusted+200, -- opacity of fog above and at this altitude will be zero
	bottom = minheightAdjusted, -- no fog below this altitude
	fade_alt = minheightAdjusted, -- fog will linearly fade away between this and "height", should be between height and bottom
	scale = minheight/scalemult, -- how large will the clouds be
	opacity = 0.35, -- what it says
	clamp_to_map = false, -- whether fog volume is sliced to fit map, or spreads to horizon
	sun_penetration = 20000, -- how much does the sun penetrate the fog
}
...
function widget:DrawWorld()
	if((select(2,spGetCameraPosition())) >= minheightAdjusted) then
	DrawClouds()
	end
end
