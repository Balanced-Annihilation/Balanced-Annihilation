...
function UnitDetected( unitID, allyTeam, teamId )
	...
	for i=1, #udef.weapons do
		if ( currentModConfig["unitList"][udef.name] == nil or currentModConfig["unitList"][udef.name]["weapons"][i] == nil ) then
			...
			--add weapon to list
			foundWeapons[i] = {type = type, range = range, weaponDefID = udef.weapons[i].weaponDef, color1 = color1, color2 = color2}
...
function UpdateButtons()
    ...
    buttonConfig["posPercRight"] = math.clamp(buttonConfig["posPercRight"],
                                              buttonConfig["currentWidth"] / state["screenx"],
                                              1 - ((buttonConfig["currentWidth"] + xSpace) / state["screenx"]))

    buttonConfig["posPercBottom"] = math.clamp(buttonConfig["posPercBottom"],
                                               (buttonConfig["currentHeight"] * 3 + ySpace * 2) / state["screeny"],
                                               1)
...
function DrawButtonGL(text, xmin, ymin, xmax, ymax, enemy, enabled)
    ...
    glTexture(false)
    Spring.Draw.Rectangle(xmin, ymin, xmax, ymax, {color=bgColor})

    -- draw icon
    glTexture( ":n:" .. text)
    local texBorder = 0.71875
    Spring.Draw.Rectangle(xmin, ymin, xmax, ymax, {texcoords={0, 1-texBorder, texBorder, 1-texBorder, texBorder, 1, 0, 1}, border=1, bordercolor=buttonConfig["borderColor"]})
    glTexture(false)
end
...
function DrawRanges()
	...
	for test, def in pairs(defences) do
		...
		for i, weapon in pairs(def["weapons"]) do
			if CheckDrawTodo(def, i) and spIsSphereInView(def["pos"][1], def["pos"][2], def["pos"][3], weapon["range"]) then
				color = weapon["color1"]
				range = weapon["range"]
				...
				glDrawGroundCircle(def["pos"][1], def["pos"][2], def["pos"][3], weapon["range"], lineConfig["circleDivs"], color[1], color[2], color[3], lineConfig["alphaValue"], lineConfig["lineWidth"], weapon.weaponDefID)
				...
				if ( ( weapon["type"] == 4 )
					...
					--air and ground: draw 2nd bigger circle, so they do not overlap
					glDrawGroundCircle(def["pos"][1], def["pos"][2], def["pos"][3], weapon["range"] + 3, lineConfig["circleDivs"], weapon["color2"][1], weapon["color2"][2], weapon["color2"][3], lineConfig["alphaValue"], lineConfig["lineWidth"], weapon.weaponDefID)
				end
			end
		end
	end
...
function widget:TweakMouseMove(x,y,dx,dy,button)
    ...
    buttonConfig["posPercRight"] = buttonConfig["posPercRight"] + dx / state["screenx"]
    buttonConfig["posPercBottom"] = buttonConfig["posPercBottom"] + dy / state["screeny"]
...
function widget:TweakDrawScreen()
    ...
    Spring.Draw.Rectangle(xPosMin, yPosMin, xPosMax, yPosMax, {color={0,0,1,0.5}})
end

function widget:TweakIsAbove(x,y)
    return GetButton(x, y) ~= nil
end
