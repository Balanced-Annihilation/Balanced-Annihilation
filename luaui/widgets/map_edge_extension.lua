...
local function SetupShaderTable()
  shaderTable = {
	  ...
	  vertex = [[
		...
		alpha = 1.0;
		#ifdef curvature
			if(mirrorX != 0.0) worldPos.y -= pow(abs(worldPos.x-left*mirrorX)/100.0, 1.5);
			if(mirrorZ != 0.0) worldPos.y -= pow(abs(worldPos.z-up*mirrorZ)/100.0, 1.5);
			alpha = 0.0;
		...
		fogFactor = 1.0;
			...
			float fogCoord = length(gl_ClipVertex.xyz);
				fogFactor = ((gl_Fog.end/1.1 - (fogCoord)) * gl_Fog.scale/2)-0.1; // gl_Fog.scale == 1.0 / (gl_Fog.end - gl_Fog.start)
			fogFactor = (fogFactor/(fogFactor/(clamp(fogFactor*0.2, fogFactor*0.5, fogFactor*1)/3))*5);
		#endif
	...
	local function doMap(dx,dz,sx,sz)
		...
		for x=0,Game.mapSizeX-Scale,Scale do
			...
			for z=sten[ind+1], sten[ind+2], (1+(-ind*2))*Scale do
				zv = abs(dz+z)+sz
				TexCoord(1/xm0/mapSizeX, 1/z/mapSizeZ)
       -- Normal(GetGroundNormal(xm0,z))
        h = sggh(xm0,z)
				Vertex(xv0,h,zv)
				TexCoord(1/xm1/mapSizeX, 1/z/mapSizeZ)
