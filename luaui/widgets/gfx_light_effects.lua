                ...
                if lightParams.beam then --BEAM type
                    ...
                    if
                        not (lightParams.groundHeightLimit and
                            lightParams.groundHeightLimit < (y - math.max(Spring.GetGroundHeight(y, y), 0)))
                     then
...
function GadgetWeaponExplosion(px, py, pz, weaponID, ownerID)
    ...
    if
        wepconf ~= nil and
            (not wepconf.noheatdistortion and
                ((weaponConf[weaponID].wtype ~= "MissileLauncher" and weaponConf[weaponID].wtype ~= "TorpedoLauncher") or
                    wepconf.aoe > 150))
     then
        local params
