...
local fontSize = 10		-- is caclulated somewhere else anyway
local fontSizePercentage = 0.5 -- fontSize * X = actual fontsize
...
function ReGenerateBackgroundDisplayList()
	...
	for lineCount=1,prevNumLines do
		...
		glColor(colour)
		if lineCount > 2 then
			RectRound(boxSizes.x.min, boxSizes.y.max -lineCount*fontSize, boxSizes.x.max, boxSizes.y.max -(lineCount-1)*fontSize, 3.5*widgetScale, 1,1,1,1, {colour[1],colour[2],colour[3],colour[4]}, {colour[1],colour[2],colour[3],colour[4]*2})
...
function ReGenerateTextDisplayList()
	...
	font:Begin()
		...
		for _, headerName in ipairs(header) do
			font:Print(headerRemap[headerName][1], baseXSize + columnSize*colCount, baseYSize-lineCount*fontSize, (fontSize), "dco")
			font:Print(headerRemap[headerName][2], baseXSize + columnSize*colCount, baseYSize-(lineCount+1)*fontSize, (fontSize), "dco")
			colCount = colCount + 1
		end
		...
		for _, allyTeamData in ipairs(teamData) do
			for _, teamData in ipairs(allyTeamData) do
				...
				for _, varName in ipairs(header) do
					...
					font:Print(color..value, baseXSize + columnSize*colCount, baseYSize-lineCount*fontSize, (fontSize), "dco")
					colCount = colCount + 1
				end
